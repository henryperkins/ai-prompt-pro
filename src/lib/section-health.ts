...
export function getSectionHealth(
  ...
  const hasValue = (value: string, minLength = 1) =>
    value.trim().length >= minLength;
  const hasCoreIntent =
    hasValue(config.originalPrompt, 8) || hasValue(config.task, 8);

  const contextStructuredCount = Object.values(
    config.contextConfig.structured,
  ).filter((value) => typeof value === "string" && hasValue(value, 2)).length;
  ...
  const hasRag =
    ...
    hasValue(config.contextConfig.rag.vectorStoreRef, 2);
  const hasIntegrations =
    config.contextConfig.databaseConnections.length > 0 || hasRag;
  ...
  const builderSignalCount =
    ...
    hasValue(config.customFormat, 2) ||
    config.lengthPreference !== "standard"
      ? 1
      : 0) +
    (hasValue(config.examples, 12) ? 1 : 0) +
    (config.constraints.length > 0 || hasValue(config.customConstraint, 2)
      ? 1
      : 0);
  ...
  const rawBuilderState = resolveState(builderSignalCount, {
    inProgress: 1,
    complete: 4,
  });
  const builderState =
    rawBuilderState === "complete" && !hasCoreIntent
      ? "in_progress"
      : rawBuilderState;

  const rawContextState = resolveState(contextSignalCount, {
    inProgress: 1,
    complete: 3,
  });
  const contextHasMinimumCoreSignals =
    contextCoreSignals.hasObjective && contextCoreSignals.hasBackground;
  const contextState =
