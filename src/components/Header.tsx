    ...
    <header className="border-b border-border bg-card/80 backdrop-blur-sm sticky top-0 z-50">
      <div className="container mx-auto flex items-center justify-between h-12 sm:h-14 px-3 sm:px-4">
        ...
        <nav className="flex items-center gap-0.5 sm:gap-1">
          <Button variant="ghost" size="sm" onClick={onOpenTemplates} className="gap-1.5 sm:gap-2 h-8 sm:h-9 px-2 sm:px-3">
            <BookOpen className="w-4 h-4" />
            <span className="hidden sm:inline text-sm">Templates</span>
          </Button>
          <Button variant="ghost" size="sm" onClick={onOpenHistory} className="gap-1.5 sm:gap-2 h-8 sm:h-9 px-2 sm:px-3">
            <History className="w-4 h-4" />
            <span className="hidden sm:inline text-sm">History</span>
          </Button>
          <Button variant="ghost" size="icon" onClick={onToggleTheme} aria-label="Toggle theme" className="w-8 h-8 sm:w-9 sm:h-9">
