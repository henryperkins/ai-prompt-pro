...
- `enhance-prompt` proxies SSE from the Python agent service; keep SSE event format compatible with the `streamEnhance` parser (expects `data: ...` and `data: [DONE]`).
- Agent service (`agent_service/main.py`) is a FastAPI app using Microsoft Agent Framework + Azure OpenAI Responses API and streams Chat Completions-style deltas for compatibility.
...
- Agent service: `AZURE_OPENAI_*` variables (see `agent_service/README.md`).
