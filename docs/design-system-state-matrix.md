...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Button | Complete: primary/secondary/tertiary/destructive/icon-only | Complete: `sm`, `icon` | Storybook + route runtime | Storybook | Route axe + Storybook | Components showcase toggles and table pagination | Partial: runtime + disabled link regression | Complete: loading story | Partial: destructive tone only | n/a | Partial: inherits global motion policy | Complete for primary shell/community actions | `src/components/base/buttons/button.stories.tsx`, `src/test/accessibility-axe.test.tsx`, `src/pages/ComponentsShowcase.tsx` |
| ButtonUtility | Complete: secondary/tertiary icon-only button and link surfaces | Complete: `xs`, `sm` mobile-first touch targets | Storybook + showcase runtime | Storybook | Unit + route axe | n/a | Complete: disabled link/button semantics | n/a | n/a | n/a | Partial: inherits global motion policy | Complete on showcase/feed utility actions | `src/components/base/buttons/button-utility.stories.tsx`, `src/test/button-utility.test.tsx`, `playwright/design-system.mobile.spec.ts`, `src/pages/ComponentsShowcase.tsx` |
| ButtonGroup | Partial: segmented text + icon-leading controls | Complete: `sm`, `md`, `lg` mobile-first min-height contract | Storybook + showcase runtime | Storybook | Unit + route runtime | Complete: selected/unselected states via showcase and unit test | Partial: no dedicated disabled story yet | n/a | n/a | n/a | Partial: inherits global motion policy | Complete on showcase segmented controls | `src/components/base/button-group/button-group.stories.tsx`, `src/test/button-group.test.tsx`, `src/test/components-showcase-blocks.test.tsx`, `playwright/design-system.mobile.spec.ts`, `src/pages/ComponentsShowcase.tsx` |
...
| Label | Partial: default/required/tooltip/long-label wrapper states | Complete: public wrapper preserves primitive `text-sm` -> `sm:text-base` contract | Storybook + field runtime | Partial: tooltip trigger story only | Unit + field runtime | n/a | Partial: inherited disabled treatment only | n/a | n/a | n/a | Partial: inherits global motion policy | Complete on shared form fields | `src/components/base/label.stories.tsx`, `src/test/label.test.tsx`, `src/components/base/input/input.tsx`, `src/components/base/select/select.tsx`, `src/components/base/textarea/textarea.tsx` |
| Dialog/Drawer triggers | Partial: dialog + drawer trigger patterns | Partial: mobile drawer density in stories/runtime | Storybook + runtime | Storybook | Route axe | Runtime only | Partial: runtime only | n/a | n/a | n/a | Partial: inherits global motion policy | Complete: anonymous + signed-in community drawer trigger targets meet shell standard | `src/components/base/dialog.stories.tsx`, `src/components/base/drawer.stories.tsx`, `src/test/accessibility-axe.test.tsx`, `playwright/community.mobile.spec.ts` |
...
| ProfileHero | Complete: viewer/following/own-profile wrapper states | Complete: hero surface + stat-strip density | Storybook + showcase route | Runtime only | Route axe via signed-in dynamic profile route | Follow button runtime | Partial: follow pending runtime | n/a | n/a | n/a | Partial: inherits global motion policy | Complete on community route shell spacing and signed-in mobile profile coverage | `src/components/community/ProfileHero.stories.tsx`, `src/pages/ComponentsShowcase.tsx`, `src/pages/Profile.tsx`, `src/test/community-routes-accessibility-axe.test.tsx`, `playwright/community.mobile.spec.ts` |
...
| --- | --- | --- |
| Storybook stories covering each row in the Core State Matrix | `src/components/base/*.stories.tsx`, `src/components/base/button-group/button-group.stories.tsx`, `src/components/base/buttons/button-utility.stories.tsx`, `src/components/base/label.stories.tsx`, `src/components/PageShell.stories.tsx`, `src/components/community/ProfileHero.stories.tsx` | Yes |
| Unit/integration tests for focus, invalid, and disabled behavior | `src/test/button-utility.test.tsx`, `src/test/button-group.test.tsx`, `src/test/label.test.tsx`, `src/test/components-showcase-blocks.test.tsx`, `src/test/accessibility-axe.test.tsx`, `src/test/community-routes-accessibility-axe.test.tsx`, `src/test/progress-indicators.test.tsx` | Yes |
| Mobile Playwright evidence for touch target and overflow behavior | `playwright/design-system.mobile.spec.ts`, `playwright/community.mobile.spec.ts` | Yes |
